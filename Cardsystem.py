...
class ProjectCardWidget(QFrame):
    ...
    def __init__(self, project_data):
        ...
        id_label.setStyleSheet("""
            ...
            background-color: #1a332d; border-radius: 4px; padding: 3px 8px; border: none;
        """)
        id_row_layout.addWidget(id_label)
        
        mode = project_data.get("mode_of_procurement") or "N/A"
        mode_label = QLabel(f"⚙️ {mode}")
        mode_label.setStyleSheet("""
            color: #ffaa00; font-size: 11px; font-weight: bold; 
            background-color: #3a2a1a; border-radius: 4px; padding: 3px 8px; border: none; margin-left: 5px;
        """)
        id_row_layout.addWidget(mode_label)
        
        p_date = project_data.get("project_date") or "N/A"
        date_label = QLabel(f"📅 {p_date}")
        date_label.setStyleSheet("""
            color: #a0a0b0; font-size: 11px; 
            background-color: #22222a; border-radius: 4px; padding: 3px 8px; border: none; margin-left: 5px;
        """)
        ...
        title_label.setStyleSheet("color: #ffffff; font-size: 14px; font-weight: bold; border: none; background: transparent;")
        left_layout.addWidget(title_label)
        
        meta_layout = QHBoxLayout()
        div_label = QLabel(f"🏢 {project_data.get('bureau_division_name', 'N/A')}")
        div_label.setStyleSheet("color: #a0a0b0; font-size: 11px; border: none; background: transparent;")
        focal_label = QLabel(f"👤 {project_data.get('focal_person', 'N/A')}")
        focal_label.setStyleSheet("color: #a0a0b0; font-size: 11px; margin-left: 15px; border: none; background: transparent;")
        ...
        mid_title.setStyleSheet("color: #00ffcc; font-size: 11px; font-weight: bold; border: none; background: transparent;")
        ...
        saro_label.setStyleSheet("color: #a0a0b0; font-size: 11px; border: none; background: transparent;")
        ...
        ors_label.setStyleSheet("color: #a0a0b0; font-size: 11px; border: none; background: transparent;")
        ...
        if supplier:
            ...
            sup_label.setStyleSheet("color: #00ffcc; font-size: 11px; font-weight: bold; border: none; background: transparent;")
        else:
            sup_label = QLabel("🤝 Supplier: Pending award")
            sup_label.setStyleSheet("color: #a0a0b0; font-size: 11px; border: none; background: transparent;")
        ...
        status_label = QLabel(project_data.get("status", "Initiated"))
        status_color = "#33ccff"
        status_bg = "#1e3a4a"
        ...
        if "warranty" in status_lower or "complete" in status_lower:
            progress_val = 100
        elif "deliver" in status_lower or "implement" in status_lower:
            progress_val = 80
        ...
        if contract_amount > 0:
            ctr_val = QLabel(f"Awarded: ₱{contract_amount:,.2f}")
...
class ProjectDetailDialog(QDialog):
    ...
    def build_timeline(self):
        ...
        planning_box = QGroupBox("Phase 1: Planning & Budget")
        ...
        # Phase 2: Documents & Bidding
        bids_box = QGroupBox("Phase 2: Documents & Bidding")
        bids_box.setStyleSheet(self.groupbox_style())
        bids_layout = QGridLayout(bids_box)
        bids_layout.setContentsMargins(15, 20, 15, 15)
        bids_layout.setSpacing(10)
        
        docs = self.project_data.get("documents", [])
        if docs:
        ...
        if bids:
            b = bids[0]
            bids_layout.addWidget(wrap_lbl("<b>Bids Received:</b>"), 1, 0)
            bid_info = f"🎟️ Bid Ref: {b.get('bid_reference_no', 'N/A')} | BACSec Recd: {b.get('date_received_bacsec', 'N/A')} | PCMD Recd: {b.get('date_received_pcmd_initial', 'N/A') or 'Pending'} | Signatories: Box A: {b.get('signatory_box_a', 'None')}, Box C: {b.get('signatory_box_c', 'None')}"
            bids_layout.addWidget(wrap_lbl(bid_info), 1, 1, 1, 3)
            
            bids_layout.addWidget(wrap_lbl("<b>Purchase Request:</b>"), 2, 0)
            pr_val = b.get('purchase_request_no') or "N/A"
            bids_layout.addWidget(wrap_lbl(pr_val), 2, 1)
            
            bids_layout.addWidget(wrap_lbl("<b>PhilGEPS Posting:</b>"), 2, 2)
            ...
            if "Auto-detect" in phil_val:
                ...
                bids_layout.addWidget(wrap_lbl(decision), 2, 3)
            else:
                bids_layout.addWidget(wrap_lbl(phil_val), 2, 3)
                
            bids_layout.addWidget(wrap_lbl("<b>Abstract of Quotations:</b>"), 3, 0)
            abs_val = b.get('abstract_of_quotations_no') or "N/A"
            bids_layout.addWidget(wrap_lbl(abs_val), 3, 1)
            
            bids_layout.addWidget(wrap_lbl("<b>BAC Resolution:</b>"), 3, 2)
            ...
            bids_layout.addWidget(wrap_lbl("<b>Bids:</b>"), 1, 0)
            bids_layout.addWidget(wrap_lbl("No bidding details recorded yet."), 1, 1, 1, 2)
            ...
            bids_layout.addWidget(add_bid_btn, 1, 3, 1, 1, Qt.AlignRight | Qt.AlignVCenter)
            
        timeline_layout.addWidget(bids_box)
        
        # Phase 3: Contract & Supplier
        contracts = self.project_data.get("contracts", [])
        contract_box = QGroupBox("Phase 3: Contract Award & Supplier Details")
        ...
        if contracts:
            ...
            contract_layout.addWidget(wrap_lbl("<b>Contract Amount:</b>"), 2, 2)
        ...
        deliv_box = QGroupBox("Phase 4: Deliverables & Milestones")
        ...
        # Phase 5: Payments
        pay_box = QGroupBox("Phase 5: Payments & Disbursements")
        pay_box.setStyleSheet(self.groupbox_style())
        pay_layout = QVBoxLayout(pay_box)
        pay_layout.setContentsMargins(15, 20, 15, 15)
        ...
        if contracts:
            ...
            if has_deliv_milestones:
                ...
                pay_layout.addWidget(add_pay_btn)
            else:
                pay_layout.addWidget(wrap_lbl("No payments recorded yet."))
                pay_layout.addWidget(wrap_lbl("<i>Add Deliverable milestones first.</i>"))
        else:
            pay_layout.addWidget(wrap_lbl("No payments recorded yet."))
            pay_layout.addWidget(wrap_lbl("<i>Complete Contract phase to proceed.</i>"))
            
        timeline_layout.addWidget(pay_box)
        
        # Phase 6: Warranties
        warranty_box = QGroupBox("Phase 6: Warranty & Retention Release")
